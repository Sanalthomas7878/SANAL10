...
ALUMINUM_IMAGE_URL = "/static/images/aluminum-rolls-factory.svg"
COPPER_IMAGE_URL = "/static/images/copper-wire-spool.jpeg"
METAL_IMAGE_URL = "/static/images/metal-scrap-yard.jpeg"
PLASTIC_IMAGE_URL = "/static/images/plastic-scrap-mix.webp"
E_WASTE_IMAGE_URL = "/static/images/e-waste-recycle.jpeg"
MOBILE_IMAGE_URL = "/static/images/mobile-scrap-phones.avif"
PAPER_IMAGE_URL = "/static/images/paper-scrap-stack.jpeg"
...
def create_app(test_config=None):
    ...
    def parse_base_price(raw_value):
        try:
            base_price = Decimal(str(raw_value or "0"))
            if base_price < 0:
    ...
    def seed_defaults():
        categories = [
            ...
            },
            {
                ...
                "base_price": 18,
                "image_url": PAPER_IMAGE_URL,
            },
        ]

        for item in categories:
            existing_category = ScrapCategory.query.filter_by(name=item["name"]).first()
            if not existing_category:
                db.session.add(ScrapCategory(**item))
                continue
            if item["name"] in {"Aluminum Scrap", "Copper Scrap", "Metal Scrap", "Plastic Scrap", "E-Waste", "Mobile Scrap", "Paper Scrap"} and existing_category.image_url != item["image_url"]:
                existing_category.image_url = item["image_url"]
    ...
    def register():
        if request.method == "POST":
            ...
            if not phone.isdigit() or len(phone) > 10:
                flash("Phone number must contain only digits and cannot be more than 10 numbers.", "error")
                return redirect(url_for("register"))
    ...
    def create_category():
        ...
        base_price = parse_base_price(base_price_raw)
    ...
    def update_category(category_id):
        ...
        duplicate_category = ScrapCategory.query.filter(
            db.func.lower(ScrapCategory.name) == name.lower(),
            ScrapCategory.id != category.id,
        ).first()
